...
Keyboard suppression only targets numeric keys and Enter after Raw Input confirms that the selected reader is producing input. It is a focused input-protection fallback, not the primary source detection mechanism. The primary source check is always the Raw Input device path, and card composition is based only on selected-reader Raw Input events so repeated digits are not dropped by suppression de-duplication. After a selected reader source is confirmed, card validity is based on card length and the reader's Enter/timeout behavior, not keyboard input speed. If the reader's Enter key is not delivered reliably, selected-reader input is also flushed by timeout as a fallback.
