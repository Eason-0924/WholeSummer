...
- 2.0.2 發布。
- LINE 通知發送改為先建立 `PENDING` 紀錄，再呼叫 LINE API，最後以獨立交易更新為成功或失敗，避免外部訊息已送出但資料庫交易回滾導致紀錄消失。
- LINE 通知紀錄唯一鍵改為包含 `line_user_id`，同一通知可分別記錄多位家長或多個 LINE 收件者的發送結果。
- LINE Push、合併通知、請假通知、出席通知與 Webhook Reply 共用可靠發送流程，失敗時會保存 LINE API 錯誤訊息或例外原因。
- 遲到提醒會略過已核准請假的學生，並會檢查學生是否在該課程開始時已到班，即使之後簽退也不會補送該堂課遲到提醒。
- 新增 LINE 發送可靠性與遲到提醒判斷測試，涵蓋失敗紀錄、合併提醒與已到班後簽退情境。

## 2.0.1 更新內容

- 2.0.1 發布。
- 修正學生出席紀錄的班級欄位在 2.0.0 改為可為空後，仍需要保留 JPA 關聯供學生明細、通知與既有資料顯示使用。
- 學生出席查詢會一併載入班級資料，降低學生明細、開放簽退紀錄與出席列表在 EC2 環境中出現延遲載入或班級名稱遺失的機率。
- 刷卡簽退回應若出席紀錄沒有班級資料，會安全顯示為「無」，避免簽退流程因空班級而失敗。
