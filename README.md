...
4. 若家長透過 LINE 請假且教師已確認，點名頁會顯示「已確認請假」，該學生的出席狀態、到班時間、簽退時間與備註會鎖定，避免被手動覆蓋。
5. 上課時間表可切換「固定課表」與「當週課表」。固定課表顯示班級原始週期課程；當週課表會整合原課程、補課、調課與已取消課程。
6. 當週課表可勾選是否顯示已取消課程，並以標籤區分補課、調課與取消課程。
7. 課表可依科目、老師或年級套用不同標記顏色，並匯出 JPEG、DOCX 或 XLSX。
8. 班級網址會使用系統產生的代稱，班名或資料調整後仍可保持較穩定的頁面路徑。

### 學生請假與 LINE LIFF 家長端

1. 系統管理者需先在 LINE Developers 建立 LIFF 頁面，並設定 `LINE_LIFF_ID` 與 `LINE_LIFF_CHANNEL_ID` 環境變數。
2. 家長需先完成 LINE 綁定。補習班可在 LINE 通知中心為學生產生綁定碼，家長於官方帳號聊天室輸入「綁定 123456」完成綁定。
